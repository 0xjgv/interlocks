"""Step defs for tests/features/harness_tasks.feature.

Each scenario builds a minimal self-contained project on tmp_path and shells
out to `python -m harness.cli <task>` — same entry point the installed CLI
...
scenarios("../features/harness_tasks.feature")
...
def _run_in_project(cmd: str, project_root: Path) -> CliResult:
    # cmd starts with "harness <subcmd> …"; drop the "harness" sentinel.
    _, *argv = cmd.split()
