"""Step defs for tests/features/harness_stages.feature.

End-to-end stage smoke tests: materialize a minimal inline project, shell out
to ``python -m harness.cli <stage>``, and assert exit-code + output-shape
contracts. Mirrors the ``_run_harness`` pattern in ``test_harness_cli.py`` but
...
from tests.step_defs.conftest import make_tmp_project, run_harness_in_cwd

scenarios("../features/harness_stages.feature")
