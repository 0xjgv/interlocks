...
Feature: interlocks meta commands
  ...
  I want setup and scaffold modes to behave safely
  ...
  Scenario: Acceptance is a silent no-op when features/ is missing
    Given a tmp project with no features directory
    When I run "interlocks gate acceptance" in the tmp project
    Then the command exits successfully
  ...
  Scenario: init --acceptance scaffolds the canonical layout idempotently
    ...
    And the output contains "next: run `interlocks gate acceptance`"
