@agents
Feature: Register interlocks usage via `interlocks setup --agents`
  As a user adopting interlocks in an existing repo
  I want `interlocks setup --agents` to wire up AGENTS.md and CLAUDE.md
  ...
  Scenario: Missing agent docs are created with the canonical block
    ...
    When I run "interlocks setup --agents" there
