...
def _cfg(project_root: Path, **overrides: object) -> HarnessConfig:
...
def test_detect_acceptance_runner_behave_from_deps(tmp_path: Path) -> None:
    ...
    features.mkdir()
    cfg = _cfg(tmp_path, features_dir=features)
    pyproject = {"dependency-groups": {"dev": ["behave>=1.2"]}}
    assert detect_acceptance_runner(cfg, pyproject) == "behave"
...
def test_detect_acceptance_runner_pytest_bdd_beats_behave_in_deps(tmp_path: Path) -> None:
    ...
    features.mkdir()
    cfg = _cfg(tmp_path, features_dir=features)
    pyproject = {"dependency-groups": {"dev": ["behave>=1.2", "pytest-bdd>=6.1"]}}
    assert detect_acceptance_runner(cfg, pyproject) == "pytest-bdd"
