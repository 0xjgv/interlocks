...
- [x] Implement `make check --fix-optimize`. *(shipped as `interlocks fix-optimize`, alias `unblock` — the engineer-facing verb; `tasks/fix_optimize.py::cmd_fix_optimize` reuses `plan.build_plan` then runs `optimize.optimize`. Self-sufficient: one run writes the full `.lintfix/` set — `plan.json` via the same `plan.write_plan_json` `fix-plan` uses, plus `optimize.json` — and absorbs the sibling outputs as opt-in flags: `--annotate` calls `fix_annotate.emit_annotations(source="optimize")` inline, `--metrics` calls `fix_metrics.aggregate_metrics`, both before `--apply` so a failed apply still surfaces hints + metrics)*
- [x] Use candidate metrics from Phase 2 and value estimates from Phase 3. *(`optimize.candidates_from_plan` reads `PlannedCandidate` cost dims; `.lintfix/replay.json` is auto-discovered — no `--stats=` needed on the golden path — and hydrates `RuleStats` for the support-pattern component of `value_for`. `--stats=<path>` overrides the location; `--no-stats` skips discovery; when stats load, the printed summary names the source file so a stale `replay.json` never changes selection silently)*
...
- [x] Add CI artifact for `.lintfix/plan.json`. *(bundled `interlocks/defaults/github_workflow.yml` now runs a single advisory `interlocks fix-optimize --annotate --metrics` step — collapsing the former `fix-plan` / `fix-annotate` / `fix-metrics` trio into one self-sufficient invocation — then an `actions/upload-artifact@v4` step uploading the entire `.lintfix/` directory — plan, optimize, replay, metrics, and escrow patches — under `lintfix-${{ github.run_id }}`, with `if-no-files-found: ignore` so runs without candidates stay green)*
- [x] Add optional PR annotation summarizing suggested fixes. *(shipped as `interlocks fix-annotate` and as `fix-optimize --annotate`; the shared core is `fix_annotate.emit_annotations(project_root, source, input_path)` — `cmd_fix_annotate` is a thin argv wrapper, `fix-optimize --annotate` calls it inline with `source="optimize"`. It reads `.lintfix/plan.json` (or `.lintfix/optimize.json`) and emits one workflow command per `(rule, file)`: `::notice file=<file>,line=1::[<rule>] auto|escrow: ...` for auto/escrow, `::warning file=<file>,line=1::` for advisory, skipped candidates emit nothing)*
- [x] Add aggregate metrics: *(shipped as `interlocks fix-metrics` and as `fix-optimize --metrics`; the shared core is `fix_metrics.aggregate_metrics(project_root) -> Path` — `cmd_fix_metrics` is a thin wrapper. It rolls up the per-run JSON files into `.lintfix/metrics.json` with `generated_at`, a `sources` truthtable, and per-section summaries — one writer, two entry points, no divergence)*
...
- [x] CI does not fail because of advisory fix suggestions. *(the single advisory `fix-optimize --annotate --metrics` step in the bundled workflow runs with `if: always()`; `fix-annotate` emits only `::notice::` / `::warning::` lines — never `::error::` — and exits 0 even when `.lintfix/plan.json` is missing; `test_missing_plan_file_exits_zero_with_no_annotations`. The inline `fix-optimize --annotate` path is also exit-code-safe: `fix_optimize._annotate` downgrades both a hard `SystemExit` from the annotator and any other exception to a warning row, so an annotation error can never change `fix-optimize`'s exit code; `test_fix_optimize_annotate_error_does_not_change_exit_code`)*
